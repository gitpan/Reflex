package Reflex::Role::PidReaper;
BEGIN {
  $Reflex::Role::PidReaper::VERSION = '0.060';
}
use Reflex::Role;

attribute_parameter pid           => "pid";
callback_parameter  cb_reaped     => qw( on pid reaped );
method_parameter    method_start  => qw( start pid _ );
method_parameter    method_stop   => qw( stop pid _ );
method_parameter    reset_pid     => qw( reset pid _ );
